...
class RollingXGBoostVolatility:
    ...
    def __init__(self,
        ...
        self.xgb_params = xgb_params or {
            ...
            "learning rate":0.05,  #new tree weight to estimate
            "subsample":0.8,
    ...
    def fit_predict(self,df):
        ...
        # Date must 
        df = df.sort_values("date").rest_index(drop=True)
...
def main(input_file_path):
    ...
    #Feature Engineering
    df = add_lagged_features(df,lag_columns,lages=[1,3,7])
    df = df.dropna().rest_index(drop = True) #delete original index
